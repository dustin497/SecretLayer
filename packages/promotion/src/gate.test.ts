...
describe("runPromotionGate", () => {
  ...
  it("approves promotion when safety passes with channel leads", async () => {
    const result = await runPromotionGate(
    ...
    assert.ok(result.plan?.leads && result.plan.leads.length >= 4);
    assert.ok(result.plan.leads.some((l) => l.channel === "changelog"));
  });
});
